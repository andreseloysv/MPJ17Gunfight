...
public class Fieldofview : MonoBehaviour
{
    ...
    void DrawFieldOfView() {
        ...
        for (int i = 0; i <= stepCount; i++) {
            ...
            if (i > 0) {
                bool edgeDSTThresholdExeceed = Mathf.Abs(oldViewCast.dst - newViewCast.dst) > edgeDstThreshold;
                if (oldViewCast.hit != newViewCast.hit || (oldViewCast.hit && newViewCast.hit && edgeDSTThresholdExeceed)) {
    ...
    EdgeInfo FindEdge(ViewCastInfo minViewCast, ViewCastInfo maxViewCast) {
        ...
        for (int i = 0; i < edgeResolveIterations; i++) {
            ...
            ViewCastInfo newViewCast = ViewCast(angle);
            bool edgeDSTThresholdExeceed = Mathf.Abs(minViewCast.dst - newViewCast.dst) > edgeDstThreshold;


            if (newViewCast.hit == minViewCast.hit && !edgeDSTThresholdExeceed)
            {
                minAngle = angle;
